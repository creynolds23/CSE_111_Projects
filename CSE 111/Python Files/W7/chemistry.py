...
Purpose: Make conversions using a dictionary and lists
Date: 10/26/22
'''
def main():
    ## Get chemical formula for a molecule from the user
    user_chemical = input('Enter the molecular formula of the sample: ')
    ...
    main_periodic_table = make_periodic_table()

    ##Call parse_formula function to convert the chemical formula given by user
    ##to a compound list that stores element symbols and the quantity
    ##of atoms of each element in the molecule
    converted_chemical = parse_formula(user_chemical, main_periodic_table)
    
    #Call the compute_molar_mass function to compute the molar mass of the molecule 
    #from the compound list
    molar_mass = compute_molar_mass(converted_chemical, main_periodic_table )
    
    #Compute the number of moles in the sample
    result_MOLES =  mass_chemical_sample /  molar_mass
    
    #Print the molar mass
    print(f'{molar_mass:.5f} grams/mole')
    
    #Print the number of moles
    print(f'{result_MOLES:.5f} moles')

    #Call get_known_molcules to display name of molecule
    get_known_molecules_table(user_chemical)
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    return molar_mass
KNOWN_MOLECULE_INDEX = 0
KNOWN_NAME_INDEX = 1
def get_known_molecules_table(known_molecule):
    ...
    known_list = known_molecules_dict
    key = known_molecule 
    if key in known_list:
        value = known_list[key]
        molecule_name = value[KNOWN_MOLECULE_INDEX]
        print(f'Your molecule name is {molecule_name}')
    else:
        print("Molecule is unknown")
